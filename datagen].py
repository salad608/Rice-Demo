"""
Training Data Generator with Peaks
Generates synthetic training data with Gaussian peaks, outputs CSV with all points,
and identifies peak ranges with detailed annotations in units.
"""

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from scipy.ndimage import gaussian_filter1d
from dataclasses import dataclass, asdict
from typing import List, Dict, Tuple
import json
from datetime import datetime
from pathlib import Path
import argparse


@dataclass
class PeakInfo:
    """Stores information about a single peak"""
    peak_id: int
    center_x: float
    center_y: float
    range_start: float
    range_end: float
    width: float
    sigma: float

    def to_dict(self) -> Dict:
        return asdict(self)


@dataclass
class DataStatistics:
    """Stores statistics about generated data"""
    min_y: float
    max_y: float
    mean_y: float
    std_y: float
    min_x: float
    max_x: float

    def to_dict(self) -> Dict:
        return asdict(self)


class TrainingDataGenerator:
    """Generate synthetic training data with peaks"""

    def __init__(
            self,
            num_points: int = 500,
            num_peaks: int = 5,
            peak_height: float = 100.0,
            noise_level: float = 0.1,
            x_min: float = 0.0,
            x_max: float = 100.0,
            seed: int = None,
    ):
        """
        Initialize the training data generator.

        Args:
            num_points: Number of data points to generate
            num_peaks: Number of peaks to generate
            peak_height: Maximum peak height (units)
            noise_level: Noise level as fraction of peak height (0.0-1.0)
            x_min: Minimum x coordinate (units)
            x_max: Maximum x coordinate (units)
            seed: Random seed for reproducibility
        """
        self.num_points = num_points
        self.num_peaks = num_peaks
        self.peak_height = peak_height
        self.noise_level = noise_level
        self.x_min = x_min
        self.x_max = x_max

        if seed is not None:
            np.random.seed(seed)

        self.x = None
        self.y = None
        self.peaks: List[PeakInfo] = []
        self.statistics: DataStatistics = None
        self.df: pd.DataFrame = None

    def generate(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Generate the training data with peaks.

        Returns:
            Tuple of (x_coordinates, y_values)
        """
        # Generate x values
        self.x = np.linspace(self.x_min, self.x_max, self.num_points)

        # Initialize y values
        self.y = np.zeros_like(self.x)

        # Calculate sigma for gaussian peaks (width of peaks)
        sigma = (self.x_max - self.x_min) / (self.num_peaks * 5)

        # Generate peaks
        peak_indices = np.sort(
            np.random.choice(self.num_points, self.num_peaks, replace=False)
        )

        for peak_id, peak_idx in enumerate(peak_indices, start=1):
            peak_x = self.x[peak_idx]

            # Create Gaussian peak
            gaussian = self.peak_height * np.exp(
                -((self.x - peak_x) ** 2) / (2 * sigma ** 2)
            )
            self.y += gaussian

            # Calculate peak range (3-sigma rule for normal distribution)
            range_start = max(self.x_min, peak_x - 3 * sigma)
            range_end = min(self.x_max, peak_x + 3 * sigma)
            peak_y = self.y[peak_idx]

            # Store peak information
            self.peaks.append(PeakInfo(
                peak_id=peak_id,
                center_x=float(peak_x),
                center_y=float(peak_y),
                range_start=float(range_start),
                range_end=float(range_end),
                width=float(range_end - range_start),
                sigma=float(sigma),
            ))

        # Add Gaussian noise
        noise = np.random.normal(
            0,
            self.noise_level * self.peak_height,
            self.num_points
        )
        self.y = self.y + noise

        # Ensure no negative values (optional)
        self.y = np.maximum(self.y, 0)

        # Calculate statistics
        self._calculate_statistics()

        # Create DataFrame
        self._create_dataframe()

        return self.x, self.y

    def _calculate_statistics(self) -> None:
        """Calculate and store data statistics"""
        self.statistics = DataStatistics(
            min_y=float(np.min(self.y)),
            max_y=float(np.max(self.y)),
            mean_y=float(np.mean(self.y)),
            std_y=float(np.std(self.y)),
            min_x=float(np.min(self.x)),
            max_x=float(np.max(self.x)),
        )

    def _create_dataframe(self) -> None:
        """Create pandas DataFrame with all data points"""
        self.df = pd.DataFrame({
            'point_id': np.arange(1, self.num_points + 1),
            'x_coordinate': self.x,
            'y_value': self.y,
        })

    def save_csv(self, filepath: str) -> str:
        """
        Save generated data to CSV file.

        Args:
            filepath: Path where CSV should be saved

        Returns:
            Path to saved CSV file
        """
        if self.df is None:
            raise ValueError("No data generated. Call generate() first.")

        Path(filepath).parent.mkdir(parents=True, exist_ok=True)
        self.df.to_csv(filepath, index=False)
        return filepath

    def save_peak_ranges(self, filepath: str) -> str:
        """
        Save peak information to JSON file.

        Args:
            filepath: Path where JSON should be saved

        Returns:
            Path to saved JSON file
        """
        if not self.peaks:
            raise ValueError("No peaks generated. Call generate() first.")

        Path(filepath).parent.mkdir(parents=True, exist_ok=True)

        output_data = {
            'generated_at': datetime.now().isoformat(),
            'configuration': {
                'num_points': self.num_points,
                'num_peaks': self.num_peaks,
                'peak_height': self.peak_height,
                'noise_level': self.noise_level,
                'x_range': [self.x_min, self.x_max],
            },
            'peaks': [peak.to_dict() for peak in self.peaks],
            'statistics': self.statistics.to_dict(),
        }

        with open(filepath, 'w') as f:
            json.dump(output_data, f, indent=2)

        return filepath

    def visualize(self, filepath: str = None) -> None:
        """
        Create a comprehensive visualization of the generated data.

        Args:
            filepath: Optional path to save the figure
        """
        if self.y is None:
            raise ValueError("No data generated. Call generate() first.")

        fig = plt.figure(figsize=(16, 10))

        # Main plot with data and peaks
        ax1 = plt.subplot(3, 1, 1)
        ax1.plot(self.x, self.y, 'b-', linewidth=2, label='Generated Data', alpha=0.8)

        # Plot peak centers
        peak_centers_x = [peak.center_x for peak in self.peaks]
        peak_centers_y = [peak.center_y for peak in self.peaks]
        ax1.scatter(peak_centers_x, peak_centers_y, color='red', s=150,
                    zorder=5, marker='*', label='Peak Centers', edgecolors='darkred', linewidth=2)

        # Highlight peak ranges
        for i, peak in enumerate(self.peaks):
            ax1.axvspan(peak.range_start, peak.range_end, alpha=0.15,
                        color='red' if i == 0 else None)
            # Add range annotations
            ax1.text(peak.center_x, peak.center_y + 10, f"P{peak.peak_id}",
                     ha='center', fontsize=10, fontweight='bold',
                     bbox=dict(boxstyle='round', facecolor='yellow', alpha=0.7))

        ax1.set_xlabel('X Coordinate (units)', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Y Value (units)', fontsize=12, fontweight='bold')
        ax1.set_title('Generated Training Data with Gaussian Peaks',
                      fontsize=14, fontweight='bold')
        ax1.grid(True, alpha=0.3, linestyle='--')
        ax1.legend(loc='upper left', fontsize=11)

        # Peak ranges detail plot
        ax2 = plt.subplot(3, 1, 2)
        colors = plt.cm.Set3(np.linspace(0, 1, len(self.peaks)))

        for peak, color in zip(self.peaks, colors):
            # Draw range as a bar
            ax2.barh(peak.peak_id - 0.4, peak.width, left=peak.range_start,
                     height=0.8, color=color, alpha=0.7, edgecolor='black', linewidth=2)
            # Add center marker
            ax2.plot(peak.center_x, peak.peak_id, 'r*', markersize=20, zorder=5)
            # Add text label
            ax2.text(peak.range_start - 2, peak.peak_id, f"P{peak.peak_id}",
                     va='center', ha='right', fontweight='bold')

        ax2.set_xlabel('X Coordinate (units)', fontsize=12, fontweight='bold')
        ax2.set_ylabel('Peak ID', fontsize=12, fontweight='bold')
        ax2.set_title('Peak Ranges Visualization', fontsize=14, fontweight='bold')
        ax2.set_yticks(range(1, len(self.peaks) + 1))
        ax2.grid(True, alpha=0.3, axis='x', linestyle='--')

        # Peak information table
        ax3 = plt.subplot(3, 1, 3)
        ax3.axis('off')

        table_data = [['Peak ID', 'Center X', 'Center Y', 'Range Start', 'Range End', 'Width', 'Sigma']]
        for peak in self.peaks:
            table_data.append([
                f"{peak.peak_id}",
                f"{peak.center_x:.3f}",
                f"{peak.center_y:.3f}",
                f"{peak.range_start:.3f}",
                f"{peak.range_end:.3f}",
                f"{peak.width:.3f}",
                f"{peak.sigma:.3f}",
            ])

        table = ax3.table(cellText=table_data, cellLoc='center', loc='center',
                          colWidths=[0.10, 0.13, 0.13, 0.15, 0.15, 0.13, 0.12])
        table.auto_set_font_size(False)
        table.set_fontsize(9)
        table.scale(1, 2.5)

        # Style header row
        for i in range(len(table_data[0])):
            table[(0, i)].set_facecolor('#4CAF50')
            table[(0, i)].set_text_props(weight='bold', color='white')

        # Alternate row colors
        for i in range(1, len(table_data)):
            for j in range(len(table_data[0])):
                if i % 2 == 0:
                    table[(i, j)].set_facecolor('#f0f0f0')

        ax3.text(0.5, 0.95, 'Peak Range Information (All Values in Units)',
                 transform=ax3.transAxes, ha='center', fontsize=12,
                 fontweight='bold', va='top')

        plt.tight_layout()

        if filepath:
            Path(filepath).parent.mkdir(parents=True, exist_ok=True)
            plt.savefig(filepath, dpi=300, bbox_inches='tight')
            print(f"✓ Visualization saved to {filepath}")

        plt.show()

    def print_summary(self) -> None:
        """Print a summary of the generated data and peaks"""
        print("\n" + "=" * 70)
        print("TRAINING DATA GENERATION SUMMARY".center(70))
        print("=" * 70)

        print(f"\n📊 Configuration:")
        print(f"  • Total Points: {self.num_points}")
        print(f"  • Number of Peaks: {self.num_peaks}")
        print(f"  • Peak Height: {self.peak_height} units")
        print(f"  • Noise Level: {self.noise_level}")
        print(f"  • X Range: {self.x_min:.2f} - {self.x_max:.2f} units")

        print(f"\n📈 Data Statistics:")
        print(f"  • Y Range: {self.statistics.min_y:.4f} - {self.statistics.max_y:.4f} units")
        print(f"  • Mean Y: {self.statistics.mean_y:.4f} units")
        print(f"  • Std Dev Y: {self.statistics.std_y:.4f} units")

        print(f"\n🎯 Peak Ranges:")
        for peak in self.peaks:
            print(f"  Peak {peak.peak_id}:")
            print(f"    - Center: ({peak.center_x:.3f}, {peak.center_y:.3f}) units")
            print(f"    - Range: [{peak.range_start:.3f}, {peak.range_end:.3f}] units")
            print(f"    - Width: {peak.width:.3f} units (±3σ, σ={peak.sigma:.3f})")

        print("\n" + "=" * 70 + "\n")


def main():
    """Command-line interface for the training data generator"""
    parser = argparse.ArgumentParser(
        description='Generate synthetic training data with Gaussian peaks',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate with default settings
  python training_data_generator.py

  # Generate 1000 points with 8 peaks
  python training_data_generator.py --num-points 1000 --num-peaks 8

  # Generate with custom parameters and save to specific directory
  python training_data_generator.py --num-points 500 --peak-height 150 --output data/
        """
    )

    parser.add_argument('--num-points', type=int, default=500,
                        help='Number of data points (default: 500)')
    parser.add_argument('--num-peaks', type=int, default=5,
                        help='Number of peaks (default: 5)')
    parser.add_argument('--peak-height', type=float, default=100.0,
                        help='Maximum peak height in units (default: 100)')
    parser.add_argument('--noise-level', type=float, default=0.1,
                        help='Noise level 0.0-1.0 (default: 0.1)')
    parser.add_argument('--x-min', type=float, default=0.0,
                        help='Minimum X coordinate (default: 0)')
    parser.add_argument('--x-max', type=float, default=100.0,
                        help='Maximum X coordinate (default: 100)')
    parser.add_argument('--output', type=str, default='training_data/',
                        help='Output directory (default: training_data/)')
    parser.add_argument('--seed', type=int, default=None,
                        help='Random seed for reproducibility')
    parser.add_argument('--no-plot', action='store_true',
                        help='Skip visualization')

    args = parser.parse_args()

    # Create generator
    generator = TrainingDataGenerator(
        num_points=args.num_points,
        num_peaks=args.num_peaks,
        peak_height=args.peak_height,
        noise_level=args.noise_level,
        x_min=args.x_min,
        x_max=args.x_max,
        seed=args.seed,
    )

    print("🔄 Generating training data...")
    generator.generate()

    # Create output directory
    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)

    # Save files
    csv_path = output_dir / 'training_data.csv'
    json_path = output_dir / 'peak_ranges.json'
    plot_path = output_dir / 'training_data_visualization.png'

    print(f"\n💾 Saving files to {output_dir}...")
    generator.save_csv(str(csv_path))
    print(f"✓ CSV saved: {csv_path}")

    generator.save_peak_ranges(str(json_path))
    print(f"✓ Peak ranges saved: {json_path}")

    # Print summary
    generator.print_summary()

    # Generate visualization
    if not args.no_plot:
        print("📊 Generating visualization...")
        generator.visualize(str(plot_path))


if __name__ == '__main__':
    main()
